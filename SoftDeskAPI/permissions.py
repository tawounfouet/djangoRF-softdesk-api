...
class IsAuthorOrReadOnly(BasePermission):
    """
    Permission personnalisée pour s'assurer que seul l'auteur peut modifier
    ou supprimer la ressource.
    """
...
class IsContributorOrReadOnly(permissions.BasePermission):
    def has_permission(self, request, view):
        ...
        return Contributor.objects.filter(
            contributor=request.user, project=project_id
        ).exists()
...
class IsOwnerOrContributorOrReadOnly(BasePermission):
    ...
    message = "Vous devez être contributeur pour accéder au project et\
    ...
    def has_object_permission(self, request, view, obj):
        # Author can do anything he is the boss!
        if obj.author == request.user:
            return True

        if view.action == "retrieve":
            # Only contributors of the project can read it (read-only)
            contributors = [
                contrib.contributor
                for contrib in Contributor.objects.filter(project=obj)
            ]  # contributions = related_name in Contributor model
